class Parser {
	...
	constructor(code: string) {
		...
		if (code.indexOf("=") > 0) {
			if (!this.isTile(code.slice(0, 2))) {
				throw new Error(
					`(${this.code}): ${code.slice(0, 2)} is not a tile`
				)
			}
	...
	fillPredator(name: string) {
		...
		if (
			this.getFullname(name[0]) &&
			name.slice(1).match(/^[abcdefgh12345678]$/)
		) {
			this.predator = `${this.getFullname(name[0])} on ${this.fileOrRank(
				name[1]
			)}`
			return
		}
		...
		throw new Error(
			`(${this.code}): Didn't fill predator with name: ${name}`
		)
	}
	...
	fillLocation(name: string) {
		if (name.indexOf("=") > 0) {
			if (!this.isTile(name.slice(0, 2))) {
				throw new Error(
					`(${this.code}): ${name.slice(0, 2)} is not a tile`
				)
			}
